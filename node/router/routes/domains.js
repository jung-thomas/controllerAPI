...
module.exports = function() {
	...
	app.get("/", async(req, res) => {
		let options = {
			method: "GET",
			json: true,
			url: global.__controller + "/v2/domains",
			auth: {}
		};
	...
	app.get("/:guid", async(req, res) => {
		let options = {
			method: "GET",
			json: true,
			url: global.__controller + "/v2/domains/" + req.params.guid,
			auth: {}
		};
		options.auth.bearer = require(global.__base + "utils/auth").getAccessToken(req);
		try {
	...
	app.get("/:guid/certificate", async(req, res) => {
		let options = {
			method: "GET",
			json: true,
			url: global.__controller + "/v2/domains/" + req.params.guid + "/certificate",
			auth: {}
		};
		options.auth.bearer = require(global.__base + "utils/auth").getAccessToken(req);
		try {
